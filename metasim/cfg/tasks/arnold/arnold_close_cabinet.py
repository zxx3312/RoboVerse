...
json_filenames = sorted(os.listdir("roboverse_data/assets/arnold/close_cabinet_merged_pickle"))
...
for filename in json_filenames:
    ...
    name = f"ArnoldCloseCabinet{object_id}{get_scale_friendly_name(scale)}Target{targetname}Cfg"
    code = f"""@configclass
class {name}(ArnoldTaskCfg):
    ...
    traj_filepath = "roboverse_data/assets/arnold/close_cabinet_merged_pickle/{filename}"
    object_usd_path = "roboverse_data/assets/arnold/sample/StorageFurniture/{object_id}/mobility.usd"

    def __post_init__(self):
        with gzip.open(f"roboverse_data/assets/arnold/close_cabinet_merged_pickle/{filename}", "rb") as f:
            data = pickle.load(f)
...
__all__ = [name for name in globals() if name.startswith("ArnoldCloseCabinet")]
