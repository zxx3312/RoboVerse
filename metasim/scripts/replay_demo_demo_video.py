...
def set_camera_pose_real(
    self,
    position: tuple[float, float, float],
    quat: tuple[float, float, float, float],
) -> None:
    camera_inst = self.env.scene.sensors[self.cameras[0].name]
    camera_inst.set_world_poses(
        positions=torch.tensor(position, dtype=torch.float32, device=self.env.device)[None, :],
        orientations=torch.tensor(quat, dtype=torch.float32, device=self.env.device)[None, :],
        convention="opengl",
    )


def main():
    # Calculate look_at position from camera position and quaternion
    pos = (-1.6546809673309326 - (-0.245), 1.83603036403656 - 1.777, 1.6000003814697266 + 0.0)
    quat = (0.6869403719902039, 0.29284438490867615, -0.2608208656311035, -0.6118231415748596)

    # Convert quaternion to forward vector using correct formula
    w, x, y, z = quat
    forward_x = 2 * (x * z + w * y)
    forward_y = 2 * (y * z - w * x)
    ...
    magnitude = (forward_x**2 + forward_y**2 + forward_z**2) ** 0.5
    forward_x /= magnitude
    forward_y /= magnitude
    forward_z /= magnitude

    # Look 5 units forward from camera position
    look_at = (pos[0] - 5 * forward_x, pos[1] - 5 * forward_y, pos[2] - 5 * forward_z)
