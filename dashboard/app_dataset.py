...
def get_config():
    with open("conf_dataset.yml") as f:
        conf = yaml.safe_load(f)
    return conf


def get_stat_json(path):
    with open(path) as f:
        stat = json.load(f)
    return stat
...
def index():
    ...
    stat_json = get_stat_json("logs_dataset/results_new.json")
    dataset_stats = {"total": {"trajectories": 0, "frames": 0}, "by_task": {}}

    for benchmark_name, stats in stat_json.items():
        dataset_stats["by_task"][benchmark_name] = {}
        for task_name, stats_ in stats.items():
            dataset_stats["total"]["trajectories"] += stats_["total_traj"]
            dataset_stats["total"]["frames"] += stats_["total_frame"]
            dataset_stats["by_task"][benchmark_name][task_name] = {
                "trajectories": stats_["total_traj"],
                "frames": stats_["total_frame"],
            }

    return render_template(
        "dashboard_dataset.html",
